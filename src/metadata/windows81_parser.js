...
module.exports = function windows81_parser(project) {
    try {
        // TODO : Check that it's not a windows81 project?
        ...
        this.windows81_proj_dir = project;
        this.jsproj_path  = path.join(this.windows81_proj_dir, jsproj_file);
        this.sln_path     = path.join(this.windows81_proj_dir, jsproj_file.replace(/\.jsproj/, '.sln'));
    } catch(e) {
        throw new Error('The provided path "' + project + '" is not a Windows 8.1 project. ' + e);
    }
    this.manifest_path  = path.join(this.windows81_proj_dir, 'package.appxmanifest');
...
module.exports.check_requirements = function(project_root) {
    events.emit('log', 'Checking Windows 8.1 requirements...');
    var lib_path = path.join(util.libDirectory, 'windows81', 'cordova',
                    require('../../platforms').windows81.version, 'windows81');
...
module.exports.prototype = {
    ...
    www_dir:function() {
        return path.join(this.windows81_proj_dir, 'www');
    },
    config_xml:function() {
        return path.join(this.windows81_proj_dir,"config.xml");
    },
    // copy files from merges directory to actual www dir
    copy_merges:function(merges_sub_path) {
        var merges_path = path.join(util.appDir(util.isCordova(this.windows81_proj_dir)), 'merges', merges_sub_path);
    ...
    staging_dir: function() {
        return path.join(this.windows81_proj_dir, '.staging', 'www');
    },
    ...
    // calls the nessesary functions to update the windows81 project
    update_project:function(cfg) {
        //console.log("Updating windows81 project...");
